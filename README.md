...
当前生产流水线不调用 LLM；摘要只使用官方标题、发布时间以及官方 RSS 自带摘要。新闻会与最近 14 天已发布记录比较，优先收录本期新增；新发布不足 8 条时，旧条目必须明确标为“持续跟踪”。三条深度剖析使用可审计的主题规则，按事实边界、传导机制、利益影响、未知项和申论转化展开，不用模型记忆补写事实。
