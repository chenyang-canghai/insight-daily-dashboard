...
- 新闻：每天 8 条重点议题、8 条证据驱动的深度剖析、独立详情页、影响链与申论转化。
...
- 主题与移动端：跟随系统、浅色、深色；360px 起响应式布局和底部导航；支持安装为 PWA 和基础离线兜底。
...
pnpm generate:pwa-icons # 重建 PWA 图标
...
当前生产流水线不调用 LLM；摘要只使用官方标题、发布时间以及官方 RSS 自带摘要。新闻会与最近 14 天已发布记录比较，优先收录本期新增；新发布不足 8 条时，旧条目必须明确标为“持续跟踪”。8 条新闻只要取得官方正文或官方摘要，就逐条生成可审计的深度剖析，按事实边界、传导机制、利益影响、未知项和申论转化展开，不用模型记忆补写事实。
...
行测采用四套主题与数值组合轮换；连续四天的 8 道题干互不重复，回归测试会阻止重新退化为固定题目。
...
**如何安装到手机桌面？** Android Chrome 使用页面安装提示或浏览器菜单中的“安装应用”；iPhone Safari 使用“分享”→“添加到主屏幕”。首次联网后会缓存必要界面和已访问页面，但离线内容可能不是最新日报。
